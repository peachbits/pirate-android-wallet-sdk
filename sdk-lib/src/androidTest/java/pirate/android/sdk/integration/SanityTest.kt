...
import pirate.android.sdk.ext.BlockExplorer
    ...
    fun testFilePaths() {
        assertTrue(
            ...
            wallet.initializer.rustBackend.dataDbFile.absolutePath.endsWith(
                "no_backup/pirate/TestWallet_${networkName}_${DatabaseCoordinator.DB_DATA_NAME}"
            )
        )
        assertTrue(
            "$name has invalid CacheDB file",
            wallet.initializer.rustBackend.cacheDbFile.absolutePath.endsWith(
                "no_backup/pirate/TestWallet_${networkName}_${DatabaseCoordinator.DB_CACHE_NAME}"
            )
        )
        assertTrue(
    ...
    companion object {
        ...
        fun wallets() = listOf(
            ...
            arrayOf(
                ...
                BlockHeight.new(PirateNetwork.Testnet, 1330000)
            ),
            // Mainnet wallet
            arrayOf(
                TestWallet(TestWallet.Backups.SAMPLE_WALLET, PirateNetwork.Mainnet),
                ...
                BlockHeight.new(PirateNetwork.Mainnet, 1000000)
            )
        )
    }
}

package pirate.android.sdk.internal.db.pending
...
internal class PiratePendingTransactionEntity(
    ...
    fun toPendingTransaction(zcashNetwork: PirateNetwork) = PendingTransaction(
        id = id,
        value = Arrrtoshi(value),
        fee = fee?.let { Arrrtoshi(it) },
    ...
    override fun equals(other: Any?): Boolean {
        ...
        other as PiratePendingTransactionEntity
    ...
    companion object {
        ...
        fun from(pendingTransaction: PendingTransaction): PiratePendingTransactionEntity {
            ...
            return PiratePendingTransactionEntity(
...
internal val PiratePendingTransactionEntity.recipient: TransactionRecipient
    get() {
        return TransactionRecipient.new(toAddress, toInternalAccountIndex?.let { Account(it) })
    }

internal fun PiratePendingTransactionEntity.isSubmitted(): Boolean {
    return submitAttempts > 0
}

internal fun PiratePendingTransactionEntity.isFailedEncoding() = raw.isEmpty() && encodeAttempts > 0

internal fun PiratePendingTransactionEntity.isCancelled(): Boolean {
    return cancelled > 0
}

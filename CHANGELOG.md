...
- The SDK's internal networking has been refactored to a separate Gradle module `lightwallet-client-lib` (and
  therefore a separate artifact) which is a transitive dependency of the Zcash Android SDK.
    - The `z.cash.ecc.android.sdk.model.LightWalletEndpoint` class has been moved to `pirate.lightwallet.client.model.LightWalletEndpoint`
    ...
    - SDK clients should avoid using generated GRPC objects, as these are an internal implementation detail and are in process of being removed from the public API.  Any clients using GRPC objects will find these have been repackaged from `pirate.wallet.sdk.rpc` to `pirate.wallet.sdk.internal.rpc` to signal they are not a public API.
...
- `pirate.android.sdk.model`:
  - `Account`
  - `FirstClassByteArray`
  - `PendingTransaction`
  - `Transaction`
  - `UnifiedSpendingKey`
- `pirate.android.sdk.tool`:
...
- `pirate.android.sdk.type.UnifiedViewingKey`

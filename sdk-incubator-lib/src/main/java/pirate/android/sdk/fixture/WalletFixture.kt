...
sealed class WalletFixture {
    ...
    abstract fun getBirthday(zcashNetwork: PirateNetwork): BlockHeight

    abstract fun getAddresses(zcashNetwork: PirateNetwork): Addresses
    ...
    object Ben : WalletFixture() {
        override val seedPhrase: String
            get() = "view yellow sponsor daring tiny exercise method connect neutral genuine piano fresh virtual loyal entry bird nerve cliff axis image toe sausage duck elephant"
        ...
        override fun getBirthday(zcashNetwork: PirateNetwork) = when (zcashNetwork.id) {
            PirateNetwork.ID_TESTNET -> {
                BlockHeight.new(zcashNetwork, 2170000L)
            }

            PirateNetwork.ID_MAINNET -> {
                BlockHeight.new(zcashNetwork, 800000L)
            }

            else -> error("Unknown network $zcashNetwork")
        }

        override fun getAddresses(zcashNetwork: PirateNetwork) = when (zcashNetwork.id) {
            PirateNetwork.ID_TESTNET -> {
                Addresses(
                    unified =
